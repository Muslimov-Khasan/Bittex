...
const FAQ = () => {
  ...
  const handleFormSubmitFaq = async (event) => {
    ...
    const response = await fetch("https://vaziat.uz/api/v1/faq", {
      method: "POST",
      headers: {
  ...
  const fetchDataFaq = async () => {
    const storedToken = localStorage.getItem("authToken");
    const response = await fetch("https://vaziat.uz/api/v1/faq/all", {
  ...
  const handleDeleteClick = async (faqItemId) => {
    const storedToken = localStorage.getItem("authToken");
    await fetch(`https://vaziat.uz/api/v1/faq/${faqItemId}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${storedToken}`,
      },
    ...
    <div className="container">
      ...
      >
        ...
        <button className="news-close-btn" onClick={closeDeleteModal}>
          ...
          <button className="delete-modal" onClick={() => handleDeleteClick(selectedFaqItem.id)}>Xa</button>
