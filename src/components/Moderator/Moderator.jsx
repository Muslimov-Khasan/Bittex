...
const Moderator = () => {
  ...
  const fetchData = async () => {
    ...
    const response = await fetch(
      `https://vaziat.uz/api/v1/products/get-all-by-check/${false}`,
      {
        method: "GET",
        headers: {
  ...
  const handleConfirmation = async () => {
    if (productId) {
      ...
      await fetch(`https://vaziat.uz/api/v1/products/${productId.id}`, {
        method: "PUT",
        headers: {
  ...
  const handleCreateReport = async (event) => {
    ...
    const response = await fetch("https://vaziat.uz/api/v1/report/create", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${storedToken}`,
